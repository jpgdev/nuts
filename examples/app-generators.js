...
var generators = module.exports = {
    ...
    User: function userGenerator(nb, options, cb) {
        ...
        async.times(nb, function(i, next) {
            ...
            if (!password) {
                var pwLength = genUtils.getRandomBetween(ranges.passwordLength);
    ...
    BasicActivity: function basicActivityGenerator(nb, options, cb) {
        ...
        */
        ...
        async.times(nb, function(i, next) {
            ...
            // var nbLikes = genUtils.getRandomBetween(ranges.likes);
            // var nbDislikes = genUtils.getRandomBetween(ranges.dislikes);
    ...
    MultipleListsActitity: function(nb, options, cb) {
        ...
        async.times(nb, function(i, next) {
            ...
            cb(err, Array.concat.apply([], results));
        });
    ...
    Note: function noteGenerator(nb, options, cb) {
        ...
        async.times(nb, function(i, next) {
            ...
            var posX = genUtils.getRandomBetween(ranges.posX);
            var posY = genUtils.getRandomBetween(ranges.posY);
            var sizeX = genUtils.getRandomBetween(ranges.sizeX);
            var sizeY = genUtils.getRandomBetween(ranges.sizeY);
    ...
    Review: function reviewGenerator(nb, options, cb) {
        ...
        async.times(nb, function(i, next) {
            ...
            var score = genUtils.getRandomBetween(ranges.score);
    ...
    */
    Media: function mediaGenerator(nb, options, cb) {
            ...
            // For episodes
            var episodesPerSeason = genUtils.getRandomBetween(ranges.episodesPerSeason);

            // Runs this in parallel to be faster when nb is high
            async.times(nb, function(i, next) {

                // Get random values within the specified range
                var imdbRating = genUtils.getRandomBetween(ranges.imdbRating);
                var imdbCount = genUtils.getRandomBetween(ranges.imdbCount);
                var nbImages = genUtils.getRandomBetween(ranges.images);
                var runtime = genUtils.getRandomBetween(ranges.runtime);
                var nbCast = genUtils.getRandomBetween(ranges.cast);
                var nbGenres = genUtils.getRandomBetween(ranges.genres);
